...
系统临时目录（或者 cmake-ide-build-pool-dir 设置的目录）下调用 CMake，
生成 [[http://clang.llvm.org/docs/JSONCompilationDatabase.html][JSON Compilation Database]] 文件，从中读取编译参数，然后为其他扩展
设置必要的信息。这个过程可以在 *Message* Buffer 看到相应的消息。
...
何为其他扩展设置信息的。
  ...
  - irony :: 通过 irony-cdb-json-add-compile-commands-path 设置项目根
             目录和 JSON Compilation Database 所在目录
