...
输出可以在 *cmake* Buffer 中看到。
  ...
  *Windows 平台注意事项*
  ...
  3. MinGW 的 CMake
     ...
     - Ninjia 生成的 Compilation JSON Database 中的 Include Path 使用
       C:/path 风格的路径，而 File Path 使用 C:\\path 风格的路径，需要
       将后者转化为前者的风格
...
#+BEGIN_SRC emacs-lisp
    (use-package cmake-ide
      ...
      ;; 使用 MSYS 工具时的补丁
      (when bw/use-msys-tools
        (setq cmake-ide-cmake-opts (concat cmake-ide-cmake-opts
                                           " " "-G Ninja"))
        (advice-add 'cmake-ide--relativize :filter-return
                    #'expand-file-name)))
#+END_SRC
