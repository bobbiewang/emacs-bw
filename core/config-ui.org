...
** 字体
  ...
  Emacs 的 font-family-list 函数可以返回字体列表，可以通过代码 (member
"Consolas" (font-family-list)) 检查某字体是否在列表里。
...
置使用了 Consolas 和微软雅黑组合，下列字体大小设置可以做到中英文等宽。
  ...
  Emacs 支持为不同的 Charset 分别设置字体。下面是一个例子。
...
#+BEGIN_SRC emacs-lisp
  (defun bw/font-installed-p (font-name)
    "检查字体 FONT-NAME 是否可用"
    (find-font (font-spec :name font-name)))

  (defun bw/setup-fonts ()
    (when (display-graphic-p)
      ;; 设置缺省字体
      (cl-loop for font in '("Maple Mono NF CN" "Consolas")
               when (bw/font-installed-p font)
      ...
      ;; 设置汉字的字体
      (cl-loop for font in '("Maple Mono NF CN" "Microsoft Yahei")
               when (bw/font-installed-p font)
               return (progn
                        (setq face-font-rescale-alist `((,font . 1.3)))
                        (set-fontset-font t '(#x4e00 . #x9fff) (font-spec :family font))))))

  (add-hook 'window-setup-hook #'bw/setup-fonts)
  (add-hook 'server-after-make-frame-hook #'bw/setup-fonts)
#+END_SRC
